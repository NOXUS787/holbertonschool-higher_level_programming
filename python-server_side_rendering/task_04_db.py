#!/usr/bin/python3
"""A Flask application that displays product data read from a JSON file,
a CSV file, or a SQLite database, selected by a query parameter and
...
def read_csv(path):
    ...
    Returns:
        list: the products, with id and price converted to numbers.
    """
    products = []
...
def read_sql(path):
    """Read a list of product dictionaries from a SQLite database.

    Args:
        path: the path to the SQLite database file.

    Returns:
        list: the rows of the Products table as dictionaries.
    """
